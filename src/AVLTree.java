...
public class AVLTree<dataType extends Comparable<? super dataType>> extends BinaryTree<dataType>
{
   ...
   public BinaryTreeNode<dataType> SearchAVL ( dataType d )
   {
      if (root == null)
         return null;
      else
         return SearchAVL (d, root);
   }
   public BinaryTreeNode<dataType> SearchAVL ( dataType d, BinaryTreeNode<dataType> node )
   {
      if (d.compareTo (node.data) == 0) 
         return node;
      else if (d.compareTo (node.data) < 0)
         return (node.left == null) ? null : SearchAVL (d, node.left);
      else
         return (node.right == null) ? null : SearchAVL (d, node.right);
   }
